...
(function (domGlobals) {
  ...
  var getPatternSet = function getPatternSet(editorSettings) {
  ...
  var isInlinePattern$1 = function isInlinePattern$1(pattern) {
  ...
  var TextWalker = function TextWalker(startNode, rootNode) {
  ...
  var nodeMatchesPattern = function nodeMatchesPattern(dom, block, content) {
  ...
  var addMarkers = function addMarkers(dom, matches) {
    ...
    var matchesWithEnds = foldr(matches, function (acc, match) {
      ...
      return acc.concat([_assign(_assign({}, match), { endMarker: endMarker })]);
    }, []);
    return foldr(matchesWithEnds, function (acc, match) {
      ...
      return acc.concat([_assign(_assign({}, match), { startMarker: startMarker })]);
    }, []);
  };
